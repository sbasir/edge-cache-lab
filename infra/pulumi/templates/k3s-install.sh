...
SSM_PREFIX="${SSM_PREFIX:-/edge-cache-lab}"
...
# Install iptables (required by k3s, not included by default)
...
# Default timeout is 120s; override with K3S_STARTUP_TIMEOUT env var (seconds)
MAX_WAIT="${K3S_STARTUP_TIMEOUT:-120}"
# Poll interval in seconds; override with K3S_POLL_INTERVAL env var
...
# Update both kubeconfigs to use private IP instead of localhost
...
if command -v aws &> /dev/null; then
    ...
    aws ssm put-parameter \
        --name "${SSM_PREFIX}/k3s/kubeconfig" \
...
# Store node token in SSM Parameter Store (for cluster joins if needed)
echo "[kubernetes] Storing node token in SSM Parameter Store..."
if command -v aws &> /dev/null; then
    NODE_TOKEN=$(cat /var/lib/rancher/k3s/server/node-token)
    aws ssm put-parameter \
        --name "${SSM_PREFIX}/k3s/node-token" \
        --value "$NODE_TOKEN" \
        --type SecureString \
        --overwrite 2>/dev/null || true
...
# Download and install kubectl (world-readable/executable)
...
# Download and install Helm (world-readable/executable)
