...
PAYLOAD=$(jq -n --arg name "$CF_RECORD_NAME" --arg content "$PUBLIC_IP" --argjson ttl "$CF_TTL" --argjson proxied "$CF_PROXIED" \
  '{type: "A", name: $name, content: $content, ttl: $ttl, proxied: $proxied}')

if [ "$DRY_RUN_FLAG" -eq 1 ]; then
  echo "DRY RUN: would set Cloudflare record $CF_RECORD_NAME -> $PUBLIC_IP"
